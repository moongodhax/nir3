...
cursor.execute("SELECT AVG(value) FROM results WHERE type = 'ready_time';")
result = cursor.fetchall()
wasm_ready_time = round(result[0][0], 2)

cursor.execute("SELECT AVG(value) FROM results WHERE type = 'used_heap_size_start';")
result = cursor.fetchall()
wasm_used_heap_size_start = round(result[0][0], 2)

cursor.execute("SELECT AVG(value) FROM results WHERE type = 'result_time';")
result = cursor.fetchall()
wasm_result_time = round(result[0][0], 2)

cursor.execute("SELECT AVG(value) FROM results WHERE type = 'used_heap_size_end';")
result = cursor.fetchall()
...
plt.tight_layout()
plt.savefig('./tests/images/plot-small.png')
